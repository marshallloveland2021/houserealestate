...
export class MessageService {
  ...
  sendMessage(senderId: number, recipientId: number, postId: number, content: string) {
  ...
  getThreadsForOwner(ownerId: number) {
    const ownerPosts: Post[] = this.dataService.getAllPosts().filter((post: Post) => post.userId === ownerId);
    ...
    for (const msg of this.messages) {
      ...
      const otherUserId = msg.senderId === ownerId ? msg.recipientId : msg.senderId;
    ...
    const threads = Array.from(threadsMap.entries()).map(([key, msg]) => {
      ...
      const otherUser = this.userService.getAllUsers().find(u => u.id === otherUserId);
